...
use prism::{Parser, semantic::SemanticAnalyzer, codegen::{CCodeGenerator, runtime::RuntimeGenerator}};
...
fn test_complete_pipeline_simple() {
    ...
    let (header, implementation) = compile_prism_source(source)
...
fn test_complete_pipeline_with_runtime() {
    ...
    assert!(temp_path.join("prism_runtime.h").exists());
    assert!(temp_path.join("prism_runtime.c").exists());
    
    // Read and validate runtime content
    let runtime_header = std::fs::read_to_string(temp_path.join("prism_runtime.h"))
        .expect("Should be able to read runtime header");
    let runtime_impl = std::fs::read_to_string(temp_path.join("prism_runtime.c"))
...
fn test_complete_pipeline_c_compilation() {
    ...
    let runtime_c = temp_path.join("prism_runtime.c");
