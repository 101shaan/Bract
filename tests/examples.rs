...
//! 
//! These tests compile example programs to ensure the Cranelift backend works.
...
/// Test the hello_world.bract example
#[test]
fn test_hello_world_example() {
    let example_path = Path::new("examples/hello_world.bract");
    ...
    let source = fs::read_to_string(example_path)
        .expect("Should be able to read hello_world.bract");
    
    let result = compile_bract_source(&source);
    assert!(result.is_ok(), "hello_world.bract should compile successfully");
    
    let bytes = result.unwrap();
    assert!(!bytes.is_empty());
}

/// Test the simple_function.bract example
#[test]
fn test_simple_function_example() {
    let example_path = Path::new("examples/simple_function.bract");
    ...
    let source = fs::read_to_string(example_path)
        .expect("Should be able to read simple_function.bract");
    
    let result = compile_bract_source(&source);
    assert!(result.is_ok(), "simple_function.bract should compile successfully");
    
    let bytes = result.unwrap();
    assert!(!bytes.is_empty());
}

/// Test compilation of a canonical Bract program
#[test]
...
fn test_all_example_files() {
    ...
    for (i, entry) in entries.enumerate() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("bract") {
            example_count += 1;
            ...
            match compile_bract_source(&source) {
                Ok(bytes) => {
                    assert!(!bytes.is_empty());
                    success_count += 1;
